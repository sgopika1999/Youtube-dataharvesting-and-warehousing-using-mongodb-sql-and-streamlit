...
st.set_page_config(page_title="youtube information")

with st.sidebar:
    options=["Home","Extract & Transfer to mongodb","Migration to SQL","Queries"]
    default_option="Home"
...
if selected== "Home":
     ...
     st.text("Python,MongoDB, Youtube Data API, MySql, Streamlit")
     st.subheader(":red[DOMAIN]")
     st.text("Social Media")
     st.subheader(":red[OVERVIEW]")
     st.markdown("Retrieving the Youtube channels data from the Google API, storing it in a MongoDB, migrating and transforming data into a SQL database,then querying the data and displaying it in the Streamlit app.")         

if selected=="Extract & Transfer to mongodb":
    st.write("Enter the youtube channel_id below:")        
